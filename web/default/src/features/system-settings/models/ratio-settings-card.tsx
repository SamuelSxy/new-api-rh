...
const modelSchema = z.object({
  ...
  VideoInputRatio: z.string().superRefine((value, ctx) => {
    const result = validateJsonString(value)
    if (!result.valid) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
  ...
  DurationBilling: z.string().superRefine((value, ctx) => {
    const result = validateJsonString(value)
    if (!result.valid) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export function RatioSettingsCard({
  ...
  const modelNormalizedDefaults = useRef({
    ...
    DurationBilling: normalizeJsonString(modelDefaults.DurationBilling),
  })
  ...
  const modelForm = useForm<ModelFormValues>({
    ...
    defaultValues: {
      ...
      DurationBilling: formatJsonForTextarea(modelDefaults.DurationBilling),
    },
  })
  ...
  const saveModelRatios = useCallback(
    async (values: ModelFormValues) => {
      const normalized = {
        ...
        DurationBilling: normalizeJsonString(values.DurationBilling),
      }

      const apiKeyMap: Record<string, string> = {
        BillingMode: 'billing_setting.billing_mode',
        BillingExpr: 'billing_setting.billing_expr',
        DurationBilling: 'billing_setting.duration_billing',
      }
